...
if( !empty ( $rows ) )
{
	$jsonData = json_decode( wp_remote_retrieve_body( wp_remote_get( RGCASHBACK__API_URL . "api/cashback_stores/json/".$project_detail[0]->subcription_id, array( 'timeout' => 120, 'sslverify'   => false ) ) ), true); 
	$result = $jsonData['response']['cashback'];
			...
			if( empty( $rg_store_exists ) )
			{
				$wpdb->insert( 
					...
					array( 
						...
						'category_ids' 				=> $row['cashback_category_id'], 
						'date' 						=> date('Y-m-d H:i:s')
					) 
				);
			}					
		}
		echo 'You successfully imported the cashback<br>';
	} else 
	{
		$qry_response = '<p style="color:red">'.$jsonData['response']['message'].'</p>';
	}
	$jsonDataCategories = json_decode( wp_remote_retrieve_body( wp_remote_get( RGCASHBACK__API_URL . "api/cashback_categories/json/".$project_detail[0]->subcription_id, array( 'timeout' => 120, 'sslverify'   => false ) ) ), true); 
...
{
	$banner_table = $wpdb->prefix.'rg_banner';
	$jsonDataBanners = json_decode( wp_remote_retrieve_body( wp_remote_get( RGCASHBACK__API_URL . "api/banners/json/".$project_detail[0]->subcription_id, array( 'timeout' => 120, 'sslverify'   => false ) ) ), true); 
